...
class Factor(object):
  ...
  def __init__(self,vars=VarSet(),vals=1.0, orderMethod='F'):
    ...
    try:
      self.t = vals.clone().to(_dtype)
    except:
      self.t = torch.tensor(vals, dtype=_dtype)
      if self.t.numel()==1: self.t=self.t.repeat(vlistdims)
      elif self.t.dim()==1 and self.nvar > 1 and orderMethod=='F': 
        ## TODO: hack to ensure linear tables are read in Fortran order, like original version
        warnings.warn('Flattened array conversions default to Fortran order, but this may change in future releases!')
        self.t=self.t.reshape(vlistdims[::-1]).permute(tuple(range(len(vlistdims)))[::-1]).reshape(vlistdims)
      else: self.t=self.t.reshape(vlistdims)
    if vlist != sorted(vlist):
      newOrder = tuple(map(lambda x:vlist.index(x), self.v))
      self.t = self.t.permute(newOrder)
  ...
  def copy(self):
    ...
    return Factor().__build(self.v.copy(),self.t.clone())  
  ...
  def changeVars(self, vars, copy=True):
    ...
    if copy: ret = Factor(v, self.t.permute(newOrder))
    else:    ret = Factor().__build(v, self.t.permute(newOrder))  # try not to copy if possible
    return ret

  def __repr__(self):
    """Detailed representation: scope (varset) + table memory location"""
    return 'Factor({:s},[0x{:x}])'.format(str(self.v),self.t.data_ptr())
  ...
  def table(self,values):
    try:
      ...
      self.t = torch.tensor(values,dtype=_dtype).reshape(self.v.dims()) #   try again using reshape

  ## Hacky thing to allow no-grad writing to tables akin to torch Parameters
  @property
  def data(self): return self.t.view(self.t.shape)
  @data.setter
  def data(self, table): self.t[:] = table[:]  ## must be the same size!
  ...
  def __getitem__(self,loc):
    ...
    if self.t.ndim == 1 or isinstance(loc, (tuple, list)):
      ...
      try:
        if self.t.ndim == 0 and loc==0: return self.t[()]
  ...
  def value(self,x):
    ...
    if self.nvar == 0: 
      try: return self.t[0]
      except: return self.t[()]   ## why sometimes [1.] and sometimes 1.?
  ...
  def valueMap(self,x):
    ...
    if self.nvar == 0: 
      try: return self.t[0]          # if a scalar f'n, nothing to index
      except: return self.t[()]      ## why sometimes [1.] and sometimes 1.?
  ...
  def __float__(self):
    ...
    if (self.nvar == 0): 
      try:    return float(self.t[0])     ## why sometimes [1.] and sometimes 1.?
      except: return float(self.t[()])
  ...
  def __abs__(self):
    ...
    return Factor().__build( self.v.copy() , torch.abs(self.t) ) 

  abs = __abs__

  def __neg__(self):
    """Return the negative of F:         G = -F  =>  G(x) = -F(x) for all x"""
    return Factor().__build( self.v.copy() , torch.negative(self.t) )

  def exp(self):
    """Return the exponential of F:      G = F.exp()  =>  G(x) = exp(F(x)) for all x"""
    return Factor().__build( self.v.copy() , torch.exp(self.t) )

  def __pow__(self,power):
    """Return F raised to a power:       G = F.power(p)  =>  G(x) = ( F(x) )^p for all x"""
    return Factor().__build( self.v.copy() , torch.pow(self.t,power) )
  ...
  def log(self):    # just use base?
    ...
    return Factor().__build( self.v.copy() , torch.log(self.t) )

  def log2(self):
    """Return the log base 2 of F:       G = F.log2()  =>  G(x) = log2( F(x) ) for all x"""
    #with torch.errstate(divide='ignore'):
    return Factor().__build( self.v.copy() , torch.log2(self.t) )

  def log10(self):
    """Return the log base 10 of F:      G = F.log10()  =>  G(x) = log10( F(x) ) for all x"""
    #with torch.errstate(divide='ignore'):
    return Factor().__build( self.v.copy() , torch.log10(self.t) )
  ...
  def __add__(self,that):
    ...
    return self.__opExpand2(that,torch.add)
  ...
  def __imul__(self,that):
    ...
    return self.__opExpand2(that,torch.multiply, out=self)

  def __div__(self,that):
    """Division of factors, e.g.,  G(x_1,x_2) = F1(x_1) / F2(x_2)"""
    #with torch.errstate(divide='ignore'):
    return self.__opExpand2(that, torch.divide)
  ...
  def __rdiv__(self,that):
    ...
    B = that if isinstance(that,Factor) else Factor([],that)
    #with torch.errstate(divide='ignore'):
    return B.__opExpand2(self, torch.divide) 
  ...
  def __idiv__(self,that):
    ...
    #with torch.errstate(divide='ignore'):
    return self.__opExpand2(that,torch.divide, out=self)
  ...
  def sum(self, elim=None, out=None):
    ...
    return self.__opReduce2(self.v & elim,torch.sum, out=out)

  def marginal(self, target, out=None):
    """Compute the marginal of F, e.g., f(X_2) = \\sum_{x_1} F(x_1,X_2) = F.marginal([X2])"""
    return self.__opReduce2(self.v - target,torch.sum, out=out)
  ...
  def lse(self, elim=None, out=None):
    ...
    if (elim is None): elim = self.v
    return self.__opReduce2(self.v & elim, torch.logsumexp, out=out)
  ...
  def argmax2(self, cvars=None, ctuple=None):
    ...
    """
    if (cvars is None):
      return self.v.ind2sub(int(self.t.argmax()))  ## "int": fix torch/numpy warning
    ax = tuple(ctuple[cvars.index(x)] if  x in cvars else slice(None) for x in self.v)
    return self.v.ind2sub(int(self.t[ax].argmax()))
  ...
  def argmin2(self, cvars=None, ctuple=None):
    ...
    if (cvars is None):
      return self.v.ind2sub(int(self.t.argmin()))
    ax = tuple(ctuple[cvars.index(x)] if  x in cvars else slice(None) for x in self.v)
    return self.v.ind2sub(int(self.t[ax].argmin()))

  def argmin(self, evidence={}):
    """Find the argmin of the factor, with partial conditioning if desired (dict version)"""
    if len(evidence)==0:
      return self.v.ind2sub(int(self.t.argmax()))
    ax = tuple(evidence[v] if v in evidence else slice(None) for v in self.v)
    return self.v.ind2sub( int(self.t[ax].argmax()) )
  ...
  def sample(self, Z=None):
    ...
    for i,it in enumerate(self.t.flatten()):
      pSoFar += it  #it[0]
      if ( pSoFar  > pDraw ):
        return self.v.ind2sub(i) #it.multi_index   # multi_index for tuple return
      #it.iternext()
  ...
  def norm(self, distance):
    ...
    elif distance == 'ukl':  return (self*self.log() - self).sum()+1.
  ...
  def distance(self, that, distance):
    ...
    elif distance == 'ukl':  return (tmp*(tmp/that).log() - tmp + that).sum()
...
# return
  ...
  def __opUnary(self,op):
    return Factor( self.v.copy() , self.t.clone() ).__opUnaryIP(op)
...
# 
  ...
  def __opExpand2(self,that,op, out=None):
    ...
    if ( (out is not None) and (out.v == vall) ): # if out can be written to directly, do so
      op( A.t.reshape(dA) , B.t.reshape(dB), out=out.t )   
    else:
      t = op( A.t.reshape(dA) , B.t.reshape(dB) )   
      if (out is None): out = Factor()
      if (len(vall)==0): t = torch.tensor([t],dtype=_dtype)
      out.__build(vall,t)
    return out
  ...
  def __opReduce2(self, elim, op, out=None):  # assumes elim <= self.v
    ...
    if elim is None: elim = self.v
    if (out is None):   # non-in-place version
      ax = tuple(self.v.index(x) for x in elim)
      vout = self.v - elim
      if len(vout)>0:
        out = Factor(self.v - elim, self.t if len(ax)==0 else op(self.t, axis=ax))    ## torch behaves differently than np!
      else:
        out = op(self.t, axis=ax)  # scalar output
...
class FactorModule(Factor):
    def __init__(self, F, xform):
        self.F = F;
        self.xform = xform
        self._XF = xform(self.F)  ## Need to pretend to be normal for super():
        super().__init__(self._XF.v, self._XF.t);
        self._XF = None           ## Now we can be read-only and dynamically computed
    @property
    def t(self):
        if self._XF is not None: return self._XF.t
        else: return self.xform(self.F).t
    @t.setter
    def t(self, table):
        if self._XF is not None: return
        else: raise ValueError("Can't set table for FactorModule; its table is dynamically derived via transformation.")
