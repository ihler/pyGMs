...
class WOGraphModel(GraphModel):
  ...
  def __init__(self, factorList=None, elimOrder=[], weights=[], copy=True, isLog=False):
    ...
    self.v_beliefs = [ None for v in self.X ]
  ...
  def weights(self, weights):
    ...
    for f in self.factors:
      self.fweights[f] /= xweights[ f.v.labels ]   # normalize the weights
  ...
  def condition(self, evidence):
    ...
    for v,x in evidence.iteritems():
      ...
      self.fweights[f] = np.array([self.weights[v]])
  ...
  def armijo(self, thetas,weights,Xi,steps,threshold=1e-4,direction=+1, optTol=1e-8,progTol=1e-8):
      ...
      f0,f1 = None, sum(bounds) #WOGraphModel.calc_bounds(thetas,weights,pri))    # init prev, current objective values
      match = reduce(lambda a,b: a&b, [th.vars for th in thetas], thetas[0].vars)
      # TODO: if all max variables, should do something simpler (max-marg matching)
      #       => same marginal computation? take log & match average
      # TODO: save marginal (belief) of Xi
      ...
      for s in range(steps):
        ...
        dT,dW = (list(l) for l in zip(*[WOGraphModel.calc_deriv(th,wt,self.priority,match,Xi) for th,wt in zip(thetas,weights)]))
        ...
        for j in range(10):
          ...
          bounds = WOGraphModel.calc_bounds(newthetas,newweights,self.priority)
          f1 = sum(bounds)
  ...
  def init(self):
    ...
    bounds = self.calc_bounds(self.factors, [self.fweights[f] for f in self.factors], self.priority)
